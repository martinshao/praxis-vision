...
业务服务只写本模块数据。依赖：workspace, skills；通过共享类型 / 服务访问上游，不反向依赖下游。UI 不直连供应商或数据库。
...
| 方法 / 路径                     | 输入                                                | 输出            |
| ------------------------------- | --------------------------------------------------- | --------------- |
| `GET /api/styles`               | query,cursor                                        | Style[]         |
| `POST /api/styles`              | name,dimensions,skillVersionIds                     | StyleVersion    |
| `GET /api/styles/:id`           | id                                                  | Style+versions  |
| `POST /api/styles/:id/versions` | dimensions,skillVersionIds,expectedVersion          | StyleVersion    |
| `POST /api/styles/compose`      | sourceVersionIds,dimensionSelections,overrides,name | NewStyleVersion |

## 数据模型

Style 与 StyleVersion；技能版本和组合来源不可变；组合只存最终维度值和来源。
