...
提示词创建虚构人物 → 查看候选 → 选择身份参考 → 命名保存 → 在拍摄中复用；改变身份另存版本。对应 F-002 / AC-002。
...
- 字段：名称、身份描述、生成参考候选、身份版本；发色 / 服饰属于方案可变造型。
- 数据：Character 容器和不可变 CharacterVersion；身份参考关联本人已生成的 Asset；造型属于拍摄方案。
...
- `GET /api/characters`：输入 query,cursor，输出 Character[]。
- `POST /api/characters`：输入 name,identityDescription,generatedAssetIds，输出 CharacterVersion。
- `GET /api/characters/:id`：输入 id，输出 Character+versions。
- `POST /api/characters/:id/versions`：输入 identityDescription,generatedAssetIds,expectedVersion，输出 CharacterVersion。
